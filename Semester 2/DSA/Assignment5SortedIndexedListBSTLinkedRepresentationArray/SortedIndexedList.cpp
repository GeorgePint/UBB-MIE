...
SortedIndexedList::SortedIndexedList(Relation r) {
    ...
    //here we create the array of empty positions
...
TComp SortedIndexedList::getElement(int i) const {
    if (i < 0 || i >= this->numberOfValues) //index out of bounds
        throw exception();
    ...
    while (this->tree[current].leftElements != i)
    {
        ...
        {
            i =i - (this->tree[current].leftElements + 1); // decrement the number of nodes from the left subtree of this node
...
void SortedIndexedList::removeNode(int nodePos, int parentPos, int nodeChildPos)
{
    if (nodePos == this->root)
        ...
        this->tree[parentPos].leftPos = nodeChildPos; // update left child reference
    else
        this->tree[parentPos].rightPos = nodeChildPos; // update right child reference
...
void SortedIndexedList::add(TComp e) {
    ...
    int current = this->root, parent = -1;
    while (current != -1)   // find where to insert the node by traversing the tree
    {
        parent = current;
        if (this->rel(e, tree[current].element))
        {
            this->tree[current].leftElements++;
            current = this->tree[current].leftPos;// go to left child
        }
        else
            current = this->tree[current].rightPos;// go to right child
    }
